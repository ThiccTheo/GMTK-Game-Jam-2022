...
Tile::Tile(const sf::Vector2i& indices): Entity(indices, bodySizeMap[EntityType::tile], OriginSpot::middleCenter)
{
}
...
void Tile::draw()
{
	...
	for (auto& tile : tiles)
	{
		...
		currentVertices[1].texCoords = sf::Vector2f(bodySizeMap[EntityType::tile].x, 0.f);
		currentVertices[2].texCoords = sf::Vector2f(bodySizeMap[EntityType::tile]);
		currentVertices[3].texCoords = sf::Vector2f(0.f, bodySizeMap[EntityType::tile].y);
